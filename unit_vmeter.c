...
static unit_vmeter_config_t vmeter_config =
{
    .gain = UNIT_VMETER_GAIN_2048MV,
...
{
    uint8_t data[ 2 ];
    esp_err_t err = core2foraws_expports_i2c_read( UNIT_VMETER_ADS1115_ADDR, reg, data, 2 );
    if ( err == ESP_OK )
    {
...
static esp_err_t vmeter_write_register( uint8_t reg, uint16_t value )
{
    ...
    return core2foraws_expports_i2c_write( UNIT_VMETER_ADS1115_ADDR, reg, data, 2 );
}

static esp_err_t vmeter_eeprom_read( uint8_t address, uint8_t *buffer, uint8_t length )
{
    return core2foraws_expports_i2c_read( UNIT_VMETER_EEPROM_ADDR, address, buffer, length );
}
...
static esp_err_t vmeter_load_calibration_for_gain( unit_vmeter_gain_t gain )
{
    ...
    if ( actual != 0 )
    {
        ...
        ESP_LOGI( TAG, "Loaded calibration: hope=%d, actual=%d, factor=%.4f", 
                 hope, actual, vmeter_config.calibration_factor );
    }
    else
    {
        vmeter_config.calibration_factor = 1.0f;
...
esp_err_t unit_vmeter_init( unit_vmeter_mode_t mode )
{
    ...
    uint16_t config = 0x8000 |  // Start bit (for single-shot)
                     ...
                     ( vmeter_config.gain << 9 ) |
                     ( vmeter_config.mode << 8 ) |
...
esp_err_t unit_vmeter_reading_get(float *voltage)
{
    ...
    *voltage = resolution * vmeter_config.calibration_factor * raw_value * VMETER_MEASURING_DIR;
